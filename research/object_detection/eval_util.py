...
def write_metrics(metrics, global_step, summary_dir, pr_values=None):
  ...
  for key in sorted(pr_values):
    pr_value = pr_values[key]
    if all(v is not None for v in pr_value.values()):
      ...
      num_thresholds = min(100, len(pr_value['precisions']))
      if num_thresholds != len(pr_value['precisions']):
        gap = len(pr_value['precisions']) // num_thresholds
        pr_value['precisions'] = np.append(pr_value['precisions'][::gap], pr_value['precisions'][-1])
        pr_value['recalls'] = np.append(pr_value['recalls'][::gap], pr_value['recalls'][-1])
        num_thresholds = len(pr_value['precisions'])
      # the pr_curve_raw_data_pb() needs the a ascending precisions array and a descending recalls array
      pr_value['precisions'].sort()
      pr_value['recalls'][::-1].sort()
      #write pr curve
      summary = summary_lib.pr_curve_raw_data_pb(
        ...
        precision=pr_value['precisions'],
        recall=pr_value['recalls'],
        num_thresholds=num_thresholds
        )
      summary_writer.add_summary(summary, global_step)
      logging.info('%s: %f', key, pr_value)
  ...
  with tf.contrib.slim.queues.QueueRunners(sess):
    try:
      ...
      for evaluator in evaluators:
        ...
        all_pr_values.update(pr_value)
      global_step = tf.train.global_step(sess, tf.train.get_global_step())

      for key, value in iter(aggregate_result_losses_dict.items()):
        all_evaluator_metrics['Losses/' + key] = np.mean(value)
  sess.close()
  return (global_step, all_evaluator_metrics, all_pr_values)
...
def repeated_checkpoint_run(tensor_dict,
  ...
  while True:
    ...
    if not model_path:
      ...
      global_step, metrics, pr_values = _run_checkpoint_once(tensor_dict, evaluators,
                                                  batch_processor,
                                                  checkpoint_dirs,
      ...
      write_metrics(metrics=metrics, pr_values=pr_values, global_step=global_step, summary_dir=summary_dir)
